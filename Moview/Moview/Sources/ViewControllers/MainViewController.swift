...
final class MainViewController: UIViewController {
  ...
  private var dataSource: RxCollectionViewSectionedAnimatedDataSource<MovieDataSection> {
    ...
    }, configureSupplementaryView: { section, collectionView, sectionType, indexPath in
      if sectionType == UICollectionView.elementKindSectionHeader {
        // Section header for recommended Movies
        ...
        supplementaryView.configure(title: "Recommend for you")
        return supplementaryView
      }
      
      if sectionType == UICollectionView.elementKindSectionFooter {
        // Footer loading view
  ...
  private func setupCollectionView() {
    ...
    collectionView.register(SectionHeaderCollectionReusableView.self,
                            ...
                            withReuseIdentifier: SectionHeaderCollectionReusableView.identifier)
    collectionView.register(CollectionViewFooterLoadingView.self,
  ...
  private func bindInputs() {
    ...
    collectionView.rx.contentOffset
      .map { [weak self] offset in
        ...
        if offset.y > self.collectionView.contentSize.height - Metric.loadNextPageThreshold {
          return true
        } else {
          return false
        }
    ...
    // Update indexPath of footer view
    collectionView.rx.willDisplaySupplementaryView
      .asDriver()
      .drive(with: self, onNext: { vc, supplementary in
        if supplementary.elementKind == UICollectionView.elementKindSectionFooter,
           let supplementaryView = supplementary.supplementaryView as? CollectionViewFooterLoadingView {
          if vc.viewModel.output.isPaging.value {
            supplementaryView.startLoading()
          } else {
            supplementaryView.stopLoading()
          }
        }
      })
      .disposed(by: disposeBag)
