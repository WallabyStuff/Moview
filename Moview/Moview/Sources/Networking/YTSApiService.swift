...
final class YTSApiService {
  ...
  public func getMovieList(limit: Int = 50, page: Int = 1) -> Single<[Movie]> {
    return Single.create { observer in
      let url = Self.basePath + "/list_movies.json?limit=\(limit)&page=\(page)"
  ...
  public func searchMovie(term: String, page: Int = 1) -> Single<[Movie]> {
    return Single.create { observer in
      ...
      let url = Self.basePath + "/list_movies.json?query_term=\(term.modifiedTerm())&page=\(page)"
      AF.request(url,
                 method: .get,
                 parameters: nil,
                 encoding: URLEncoding.default,
